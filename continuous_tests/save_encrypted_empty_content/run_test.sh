#!/bin/bash

#TODO-This test is not a right test. It is always PASS (Bug #3429)

# Test case flow:
# - Runs and connects two instances of metax.
# - Sends "node" save request for empty content (default it is saved as encrypted)
#   to the metax, running with config1.xml.
# - Sends get request for received uuid to the metax, running with config2.xml.
# - The reply will be empty, no crash should happen.

get_file=$test_dir/get_file


source ../../test_utils/functions.sh

# wait command exit code when metax was successfully finished (i.e. without Seg. fault or Assertion)
EXIT_CODE_OK=0


clean_storage_content()
{
        echo "remove uuids(except user jsons) from $1 directory"
        str=""
        for i in ${!uj_uuids[@]}; do
                str+=" -o -name ${uj_uuids[$i]}"
        done
        find $1 -type f ! \( -name storage.json $str \) -delete
}




start_metaxes()
{
        $EXECUTE -f $tst_src_dir/config2.xml &
        p2=$!
        is_peer_connected=$(peers_connect 7091 $tst_src_dir/config2.xml)
        if [ "0" != "$is_peer_connected" ];then
                echo "7090 does not connect to peers"
                kill_metax $p2
                exit;
        fi

        $EXECUTE -f $tst_src_dir/config1.xml &
        p1=$!
        is_peer_connected=$(peers_connect 7081 $tst_src_dir/config1.xml)
        if [ "0" != "$is_peer_connected" ];then
                echo "7080 does not connect to peers"
                kill_metax $p1 $p2
                exit;
        fi
        echo -n $p2 $p1 > $test_dir/processes_ids
}



main()
{
        start_metaxes
        get_user_json_uuids 2
        uuid=$(curl --data "" "http://localhost:7081/db/save/node")
        uuid=$(echo $uuid | cut -d'"' -f 4)
        curl http://localhost:7081/db/get/?id=$uuid > $get_file
        if [[ ! -f $get_file || -s $get_file ]]; then
                echo "GET request failed. get_file is not empty"
                kill_metax $p1 $p2
                exit
        fi

        sleep $TIME
        kill_metax $p1 $p2
        clean_storage_content $test_dir/storage1
        start_metaxes

        curl http://localhost:7081/db/get/?id=$uuid > $get_file
        if [[ ! -f $get_file || -s $get_file ]]; then
                echo "GET request failed after removing storage1. get_file is not empty"
                kill_metax $p1 $p2
                exit
        fi
        echo $PASS > $final_test_result;
        kill_metax $p1 $p2
}

main $@
