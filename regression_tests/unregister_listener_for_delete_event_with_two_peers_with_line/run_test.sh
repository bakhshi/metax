...
# Test case flow:
# - Run three instances of metax (metax1 -> metax2 -> metax3).
# - Send save request to metax3.
# - Send websocket connect request to running metax1 and metax2 by
#   running websocket_client.js nodejs script.
# - Send register_listener request to metax1 and metax2.
# - Send update request to the metax3.
# - Check that update event successfully delivered.
# - Send unregister_listener request to metax1 and metax2.
...
main()
{
        ...
        res=$(curl http://localhost:7091/db/unregister_listener?id=$suuid)
        expected='{"success": "listener unregistered for '$suuid'"}'
        if [[ "$res" != "$expected" ]]; then
                echo "Failed to unregister Peer2: Mismatch of expected and received response. Res = $res"
                kill_metax $p3 $p2 $p1
                exit
        fi
        ...
        expected="{\"event\":\"deleted\",\"uuid\":\"$suuid\"}"
        if [ "$data1" == "$expected" ]; then
                echo "Peer2 receive deleted event. $data1"
                kill_metax $p1 $p2 $p3
                exit;
        fi
