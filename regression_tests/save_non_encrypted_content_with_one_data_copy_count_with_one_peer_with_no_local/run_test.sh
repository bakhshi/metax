...
main()
{
        ...
        uuid=$(curl --data "$save_content" http://localhost:7081/db/save/node?local=0\&enc=0)
        ...
        res=$(curl http://localhost:7081/db/get/?id=$uuid)
        expected="{\"error\":\"Getting file failed: uuid=$uuid.\"}"
        if [ "$expected" != "$res" ]; then
                echo "Failed to save only one copy of data, expected=$expected, res=$res"
                kill_metax $p1 $p2
                exit;
        fi
