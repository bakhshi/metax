...
# - Run one instance of metax (metax1 [storage_class = 4] -> metax2).
...
# - Sends delete request to metax2.
# - Checks that update event successfully delivered.
...
main()
{
        ...
        if [ "$data" != "$expected" ]; then
                echo "Deleted event isn't received"
                kill_metax $p1 $p2 $p3
                exit;
        fi 
