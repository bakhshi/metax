...
# - Waits about 300 seconds for backup process to finish then kills metax.
...
export test_dir=$project_root/regression_tests_results/save_encrypted_1GB_file_with_one_remote_peer
tst_src_dir=$project_root/regression_tests/save_encrypted_1GB_file_with_one_remote_peer
...
main()
{
        ...
        uuids=$(ls $test_dir/storage | grep -v "$uj_uuid\|$uj_uuid.tmp" | grep -v "storage.json" | grep -v "cache")
        echo "uuids=$uuids" >> $test_output

        kill_metax $p

        # remove uuids except user json's uuid.
        find $test_dir/storage -type f ! \( -name storage.json -o -name $uj_uuid -o -name $uj_uuid.tmp \) -delete
        ...
        $CONNECT_TO_PEER "rm $peer_storage/$uj_uuid"
        kill_metax $p
