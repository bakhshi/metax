...
# - Generates random file with ~3Gb size.
# - Sends "data" save request for generated file (default it is saved as encrypted).
# - Waits about ~15m seconds for backup process to finish then kills metax.
# - Removes storage files and restart killed metax.
...
export test_dir=$project_root/regression_tests_results/save_encrypted_3GB_file_with_one_remote_peer
tst_src_dir=$project_root/regression_tests/save_encrypted_3GB_file_with_one_remote_peer
...
#else
export TIME=10
export EXECUTE="$TIMEOUT -s SIGKILL 2000 $shell_bin"
#fi
...
main()
{
        ...
        # Wait ~15m for backup process to finish
        sleep 1200

        kill_metax $p
        basename -a $test_dir/storage/????????-* | grep -v "$uj_uuid" | sort > $test_dir/local_list
        # remove uuids except user json's uuid.
        find $test_dir/storage -type f ! \( -name storage.json -o -name $uj_uuid \) -delete
        ...
        $CONNECT_TO_PEER "basename -a $remote_storage/????????-* | grep -v "$uj_uuid" 2>/dev/null | sort" > $test_dir/remote_storage_list
	...
	# temporarily disable rm commands below for debugging purposes
        #rm $save_file
        #rm $get_file
        #remove user json uuid from peer's storage
        $CONNECT_TO_PEER "rm $remote_storage/$uj_uuid"
        kill_metax $p
