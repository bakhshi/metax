...
# Test case flow:
# - Runs and connects two instances of metax with each other (metax1 -> metax2 (storage_class = 2), metax1 -> metax3).
# - Send save stream request to metax1.
# - Send get request to metax2.
# - Check that get request successed from metax3.
...
main()
{
        ...
        curl "http://localhost:7091/db/get?id=283abccd-8c1e-47c1-8795-839abcfede5c" --output $test_dir/get_file
        ...
        if [ -n "$diff" ];then
                echo "Failed getting stream from metax2"
                kill $exe_pid
                wait $exe_pid
                kill_metax $p1 $p2 $p3
                exit;
