...
# Test case flow:
# - Runs and connects three instances of metax (metax1 -> metax2, metax1 -> metax3).
# - Send save stream request to metax1.
# - Send get requests to metax1 peers.
...
main()
{
        ...
        sleep 1
        curl "http://localhost:7101/db/get?id=283abccd-8c1e-47c1-8795-839abcfede5c" --output $test_dir/get1_file&
        get_pid=$!

        curl "http://localhost:7091/db/get?id=283abccd-8c1e-47c1-8795-839abcfede5c" --output $test_dir/get2_file
        wait $get_pid
        ...
        if [ -n "$diff1" ] || [ -n "$diff2" ];then
                echo "Failed to get stream from peer2 or peer3."
                kill $exe_pid
                wait $exe_pid
                kill_metax $p1 $p2 $p3
                exit;
