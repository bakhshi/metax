...
export TIME=20
export EXECUTE="$TIMEOUT -s SIGKILL 2650  valgrind --leak-check=full --log-file=$test_dir/valgrind_result.txt $shell_bin"
else
export TIME=10
...
main()
{
        ...
        if [[ -z $d ]]; then
                ...
                res=$(cat $get_file)
                echo "Failed to get big data. Response after get is $res"
        fi
