...
# Test case flow:
# - Runs and connects four instances of metax ( metax1 -> metax2 -> metax3 -> metax4 ).
# - Sends "data" save request for random generated file (enc = 0) to metax1.
# - Sends delete request to metax4.
# - Check delete request response succeeded.
...
main()
{
        ...
        if [ "$expected" != "$res" ] ; then
                echo "Failed to delete from Peer4. Response is $res"
                kill_metax $p4 $p3 $p2 $p1
                exit;
        fi
        sleep $TIME
