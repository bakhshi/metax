...
# - Sends "path" save request for some content (default it is saved as encrypted).
...
main()
{
	...
	uuid=$(curl -D $test_dir/h --data "$save_file" http://localhost:7081/db/save/path)
	...
	d=$(diff $save_file $get_file 2>&1)
	if [[ -n $d ]]; then
                echo "Failed to get data after removing stoarge1."
	        kill_metax $p1 $p2
		exit;
	fi
