...
# - Sends delete request for received uuid to running metax2.
...
main()
{
        ...
        if [ -n "$diff" ]; then
                echo "Failed to save data in two metaxes. diff=$diff"
                kill_metax $p1 $p2
                exit;
        fi
	...
	storage_size2=$(find $test_dir/storage2 -name "????????-*" | grep -v "$str" | expr $(wc -l))
        if [ "0" != "$storage_size1" ] || [ "0" != "$storage_size2" ]; then
                echo "Failed to sync updated data : storage_size1=$storage_size1 storage_size2=$storage_size2"
                kill_metax $p1 $p2
                exit;
        fi
